...
# --- СЕКЦИЯ ДЛЯ HUGGING FACE (ВЕБ-ЗАГЛУШКА) ---
# Это нужно, чтобы сервис видел активность на порту 7860 и не "убивал" процесс
app = Flask(__name__)

@app.route('/')
def health_check():
    return "Bot is alive and monitoring Bulldrop!"

def run_flask():
    # Hugging Face по умолчанию слушает порт 7860
...
# Тот самый огромный словарь всех кейсов
CASE_MAP = {
    ...
    "DAILY": "daily", "FREE": "free-box"
}

# Инициализация OCR (с поддержкой русского и английского)
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    await message.answer("🚀 Бот запущен на 16GB RAM! Введи номер телефона для входа (+7...):")
...
async def process_phone(message: types.Message, state: FSMContext):
    ...
    await message.answer("📟 Введи код из СМС/звонка:")
...
async def process_otp(message: types.Message, state: FSMContext):
    ...
    await message.answer("✅ Авторизация успешна. Бот начал мониторинг канала!")
    await state.clear()
...
async def handle_promo_photo(message: types.Message):
    ...
    await bot.send_message(ADMIN_ID, f"🎯 Нашел код: `{code}`\n📦 Кейс: `{found_name}`\nПробую забрать...")
    ...
    try:
        ...
        await bot.send_message(ADMIN_ID, f"🎉 Успех! Выпало: **{item}**\nКейс: {found_name}")
